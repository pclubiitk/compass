...
export default function Page() {
  ...
  useEffect(() => {
    mapRef.current = new maplibregl.Map({
      ...
      style: 'https://api.maptiler.com/maps/streets-v2/style.json?key=LlBgIboBwPwSOXm52XBf',
      center: [80.2335, 26.5123],
      zoom: 15
    });

    return () => {if (mapRef.current) { mapRef.current.remove();}} // cleanup on unmount
  }, []);
   ...
   const searchPlace = async () => {
     ...
     {if (mapRef.current) { mapRef.current.flyTo({ center: coordinates, zoom: 18 })}}
     ;
  ...
  <>
    ...
    <div style={{ position: 'absolute', top: 10, left: 10, zIndex: 999, background: 'white', padding: '10px', borderRadius: '8px' }}>
      <input
        type="text"
