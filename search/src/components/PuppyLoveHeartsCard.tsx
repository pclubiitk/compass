...
export const PuppyLoveHeartsCard = ({ compact }: PuppyLoveHeartsCardProps) => {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('puppylove:draftSaved', handleDraftSaved);
  }, []);
  ...
  return (
    <div className="flex gap-4 items-start">
      ...
      <CardHeader>
        ...
        <CardTitle className={`${compact ? 'text-lg' : 'text-2xl'} text-center text-rose-500`}>Puppy Love</CardTitle>
        <CardDescription className="text-center text-rose-400">Your Hearts</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3"> 
        <div className="flex justify-between items-center rounded-lg px-3 py-2 bg-rose-100/80">
          <span className="text-sm font-medium text-rose-500">Virtual Hearts</span>
        ...
        <div className="flex justify-between items-center rounded-lg px-3 py-2 bg-rose-100/80">
          <span className="text-sm font-medium text-rose-500">Hearts Received</span>
          <span className="font-bold text-rose-500">
    ...
    {!compact && puppyLoveHeartsSent && puppyLoveHeartsSent.length > 0 && (
      <Card className="w-[250px] border-none bg-rose-50/90 shadow-[0_10px_40px_rgba(225,29,72,0.15)]">
        <CardHeader>
          ...
          <CardDescription>{puppyLoveHeartsSent.length} recipient(s)</CardDescription>
        </CardHeader>
