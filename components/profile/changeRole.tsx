...
export default function ChangeRoleDialog({
  ...
  const makeAdmin = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${BACKEND_URL}/api/admin/make-admin`, {
        method: "POST",
        credentials: "include",
      ...
      if (res.ok) {
        ...
        setRollNo("");
        onOpenChange(false);
        // Optionally trigger a refresh of the admin list
        window.dispatchEvent(new Event('adminsUpdated'));
      } else {
        const errorMsg = data.error || `Failed to make admin (${res.status})`;
